...
`;
      ...
      log(`✓ Local Supabase configured. Environment variables written to .env.local`, 'green');
    ...
    log(`✗ Failed to bootstrap local Supabase: ${err.message}`, 'red');
    return null;
  }
}
...
async function main() {
  ...
  if (!supabaseUrl || !supabaseUrl.trim()) {
    ...
    log('  ✓ NEXT_PUBLIC_SUPABASE_URL set', 'green');
  }
  ...
  if (!supabaseAnonKey || !supabaseAnonKey.trim()) {
    ...
    log('  ✓ NEXT_PUBLIC_SUPABASE_ANON_KEY set', 'green');
  }
  
  if (!serviceRoleKey) {
    warnings.push('SUPABASE_SERVICE_ROLE_KEY not set (optional, needed for admin operations)');
    log('  ⚠ SUPABASE_SERVICE_ROLE_KEY not set (optional)', 'yellow');
  } else {
  ...
  if (!healthCheck.ok && healthCheck.error?.includes('fetch')) {
    log('  ⚠ App not running or unreachable', 'yellow');
    ...
    log('  ✓ /api/health endpoint reachable', 'green');
    if (healthCheck.data) {
      const { status, checks } = healthCheck.data;
      if (status === 'ok') {
        log('  ✓ Health status: OK', 'green');
      } else {
        log(`  ⚠ Health status: ${status}`, 'yellow');
      ...
      if (!checks.db.reachable) {
        ...
        if (dbError.includes('does not exist') || dbError.includes('schema cache') || dbError.includes('table')) {
          ...
          log(`  ⚠ Database tables missing (expected before migrations): ${dbError.substring(0, 60)}...`, 'yellow');
        } else {
          warnings.push(`Database not reachable: ${dbError} - may need to run migrations`);
          log(`  ⚠ Database: ${dbError}`, 'yellow');
        }
      } else {
        log('  ✓ Database reachable', 'green');
      }
      
      if (checks.db.rlsBlocked) {
        warnings.push('Database queries blocked by RLS - may need to run migrations');
        log('  ⚠ RLS may be blocking queries', 'yellow');
      }
    }
  } else {
    warnings.push(`Health check failed: ${healthCheck.error}`);
    log(`  ⚠ Health check failed: ${healthCheck.error}`, 'yellow');
  }
  ...
  if (issues.length === 0) {
    if (warnings.length === 0) {
      ...
      log('⚠ PREFLIGHT PASSED (with warnings)', 'yellow');
      log('\nWarnings:', 'yellow');
      warnings.forEach(warning => log(`  • ${warning}`, 'yellow'));
      log('\n💡 These are expected if migrations haven\'t been run yet.', 'cyan');
...
main().catch(err => {
  log(`\n✗ Preflight script error: ${err.message}`, 'red');
  console.error(err);
  process.exit(1);
});
