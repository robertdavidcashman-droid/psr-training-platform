  ...
  return (
    <details className={cn("rounded-xl border bg-muted/20", className)} data-testid="references-details">
      ...
      >
        ...
        <span className="text-base text-muted-foreground">Click to expand</span>
      </summary>
            ...
            {references.map((r, idx) => (
              <li key={`${r.instrument}-${r.cite}-${idx}`} className="rounded-lg border bg-background p-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-base font-medium">{r.cite}</span>
                </div>
                {r.note ? (
                  <div className="mt-1 text-base text-muted-foreground">{r.note}</div>
                ) : null}
              </li>
            ))}
          </ul>
