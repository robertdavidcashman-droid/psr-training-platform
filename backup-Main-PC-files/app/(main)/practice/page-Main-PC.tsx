...
export default function PracticePage() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const normalizedCorrectAnswers = currentQuestion.correct_answer.map(a => {
      ...
      if (letterToIndex[str]) {
        return letterToIndex[str];
      }
      // If it's already a number/index, keep it
      return str;
    });
    
    const normalizedSelectedAnswers = selectedAnswers.map(a => String(a));
  ...
  const normalizedCorrectAnswers = currentQuestion.correct_answer.map(a => {
    const str = String(a).trim();
    return letterToIndexMap[str] || str;
  });
  const isCorrect = showFeedback &&
    selectedAnswers.length === normalizedCorrectAnswers.length &&
    selectedAnswers.every(answer => normalizedCorrectAnswers.includes(String(answer)));
          ...
          {showFeedback && (
            <div className={`p-5 rounded-xl border-2 ${isCorrect ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
              ...
              <div className="text-xs bg-gray-100 p-2 rounded mb-3 font-mono">
                <div>Your answer: {JSON.stringify(selectedAnswers)}</div>
                <div>Correct answer (raw): {JSON.stringify(currentQuestion.correct_answer)}</div>
                <div>Option keys: {JSON.stringify(Object.keys(currentQuestion.options))}</div>
              </div>
