              ...
              <CardContent className="p-5">
                <div className="flex flex-col md:flex-row md:items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="font-bold text-foreground text-lg leading-snug mb-2">{question.question_text}</h3>
                    {question.explanation && (
                      <p className="text-muted-foreground text-sm line-clamp-2">{question.explanation}</p>
                    )}
                  </div>
                </div>
              </CardContent>
        ...
        >
          ...
          >
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    <span className={`px-2.5 py-1 rounded-lg text-xs font-bold capitalize ${getDifficultyColor(selectedQuestion.difficulty)}`}>
                  ...
                  <CardTitle className="text-xl leading-relaxed">{selectedQuestion.question_text}</CardTitle>
                </div>
                <button 
                  onClick={() => setSelectedQuestion(null)}
            ...
            <CardContent className="space-y-6 pt-6">
              <div>
                <h3 className="font-bold text-foreground mb-3">Answer Options</h3>
                <div className="space-y-2">
                  {Object.entries(selectedQuestion.options).map(([key, value]) => {
                    const isCorrect = selectedQuestion.correct_answer.includes(key);
                    // Handle both string values and object values
                    const optionText = typeof value === 'string' 
                      ? value 
                      : (value?.text || value?.label || JSON.stringify(value));
                    return (
                      <div 
                        key={key} 
              ...
              {selectedQuestion.explanation && (
                <div className="p-5 bg-primary/5 rounded-xl border border-primary/20">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-foreground mb-2">Explanation</h3>
                      <p className="text-foreground leading-relaxed">{selectedQuestion.explanation}</p>
                    </div>
                  </div>
                </div>
              )}

              {selectedQuestion.source_refs && selectedQuestion.source_refs.length > 0 && (
                <div>
                  <h3 className="font-bold text-foreground mb-2">Sources</h3>
                  <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                    {selectedQuestion.source_refs.map((ref, idx) => (
                      <li key={idx}>{ref}</li>
                    ))}
                  </ul>
                </div>
              ...
              <Button onClick={() => setSelectedQuestion(null)} variant="navy" className="w-full">
                Close
              </Button>
            </CardContent>
          </Card>
