...
export default async function AdminDashboard() {
  ...
  // Get statistics
  const [questionsResult, usersResult, progressResult, sessionsResult] = await Promise.all([
  ...
  const pendingQuestions = questionsResult.data?.filter(q => q.status === 'pending').length || 0;
  const approvedQuestions = questionsResult.data?.filter(q => q.status === 'approved').length || 0;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <Link href="/admin/content?tab=sources">
              <Button variant="outline" className="w-full justify-start" size="lg">
                Import Sources
              </Button>
            </Link>
